...
using [moray-filter](https://www.npmjs.com/package/moray-filter) to make sure
their queries escape unknown input correctly.

Moray requires that at least one of the filters in a query be for an indexed
field, so that Moray can search the database for that. If a filter is for a
non-indexed field (or a field that is currently being indexed by
[ReindexObjects](#reindexobjects)), then Moray will apply the filter itself to
results returned by Moray. While this can sometimes be a useful feature, it can
also be extremely tricky for consumers to use properly. When filtering on
fields without an index:

- The `_count` field will be incorrect, and the actual, possible results may be
  smaller.
- Zero objects may be returned even though there might be applicable objects in
  the bucket, since it's possible for the non-indexed portion of the filter to
  be false for all of the objects returned from Postgres.
...
used to get Moray to reject filters that include non-indexed or reindexing
fields.

In addition to the fields specified in the bucket's schema, you can search on
the following internal fields:

- `_id`, an ID unique to the bucket that Moray assigns to each row
...
When Moray executes a batch operation, it will perform the operations in
the specified order. This has some important implications: because updating
a row grabs a lock on it, it is possible for two transactions to end up in
a deadlock. For example, consider executing the two following updates in
