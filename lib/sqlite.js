...
function sqliteSetup(options) {
    ...
    client.begin = function begin(level, cb) {
        ...
        // In addition, we make every transaction IMMEDIATE, because there is
        // no support for SELECT ... FOR UPDATE, and this is the only way to
        // make sure nobody else can concurrently SELECT something they intend
        // to update.  This essentially fully serialises database operations
        // for now.
        var q = client.query('BEGIN IMMEDIATE TRANSACTION');
