...
function query(sql, args) {
    ...
    var end_cb = function (err, rowCount) {
        if (ignore_events)
            return;
...
function sqliteSetup(options) {
    ...
    client.lastrowid = function lastrowid(cb) {
        var done = once(cb);
        var req = client.query('SELECT last_insert_rowid() AS _id');
        var id;
        req.once('error', done);
        req.once('row', function (row) {
            assert.object(row, 'lastrowid row');
            assert.number(row._id, 'lastrowid row._id');
            id = row._id;
        });
    ...
    client.supports = function supports(feature) {
        assert.bool(FEATURE[feature], 'feature: ' + feature);
        return (FEATURE[feature]);
    };

    client.begin = function begin(opts, cb) {
        assert.object(opts, 'begin opts');
        assert.bool(opts.rmw, 'begin opts.rmw');
        assert.string(opts.level, 'begin opts.level');
        assert.func(cb, 'begin cb');
        ...
        // In addition, if a transaction is required to have Read-Modify-Write
        // safety, we make that transaction IMMEDIATE -- this causes SQLite to
        // take an immediate RESERVED lock, preventing other concurrent IMMEDIATE
        // transactions.  This enables us RMW safety, even though SQLite does not
        // support SELECT ... FOR UPDATE.  This has the effect of serialising all
        // RMW transactions, though the SQLite WAL journalling mode allows
        // readers to continue operating concurrently.
        var q;
        if (opts.rmw) {
            q = client.query('BEGIN IMMEDIATE TRANSACTION');
        } else {
            q = client.query('BEGIN DEFERRED TRANSACTION');
        }
