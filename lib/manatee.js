...
function createPGPool(req, cb) {
        ...
        if (req.record.primary) {
                log.debug({
                        url: req.record.primary
                }, 'createPGPool: creating primary');
                pgPool.primary = pg.createClient(xtend(cfg, {
                        url: req.record.primary
                }));
        }
        ...
        if (req.record.sync) {
                log.debug({
                        url: req.record.sync
                }, 'createPGPool: creating sync');
                pgPool.sync = pg.createClient(xtend(cfg, {
                        url: req.record.sync
                }));
        }
        ...
        if (req.record.async) {
                log.debug({
                        url: req.record.async
                        }, 'createPGPool: creating async');
                pgPool.async = pg.createClient(xtend(cfg, {
                        url: req.record.async
                }));
        }
...
Manatee.prototype.watch = function watch() {
        ...
        vasync.pipeline({
                ...
                req.listener.once('error', function (err) {
                        log.debug(err, 'watch: listener emitted error');
                        self.close({skipZk: true}, function () {
                                self.watch();
                        });
                });
...
Manatee.prototype.pg = function pg(options, callback) {
        ...
        if (!this.database || !this.database.primary) {
                process.nextTick(function () {
                        callback(new Error('No database handles'));
                });
                return (undefined);
        }
