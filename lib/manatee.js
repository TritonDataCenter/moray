...
function createPGPool(req, cb) {
        ...
        var cfg = {
                ...
                queryTimeout: req.pgConfig.queryTimeout
        };
        var pgPool = {};
...
Manatee.prototype.watch = function watch() {
        ...
        vasync.pipeline({
                ...
                req.listener.once('children', function (obj) {
                        log.debug(obj, 'watch: topology changed; closing previous DB');
                        self.close({skipZk: true}, function () {
                                log.debug('watch: resetting watch');
                                self.watch();
                        });
                });
                ...
                if (self.database || self.listener) {
