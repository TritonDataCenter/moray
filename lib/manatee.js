...
Manatee.prototype._refresh = function _refresh(topology) {
    ...
    function createPGPool(opts) {
        ...
        // update the user, and table name to what moray uses.
        var pgUrl = url.parse(opts.url);
        pgUrl.auth = self._pgUser;
        pgUrl.pathname = DBNAME;
        pgUrl = url.format(pgUrl);

        log.info({pgUrl: pgUrl}, '_refresh.createPGPool: create new pg pool');
    ...
    /*
     * we only care about the primary of the shard. there are 3 conditions for
     * a primary change:
     *
     * 1) !newPrimary -- indicates no primary at all, we must d/c the old
     * primary
     * 2) !previous && newPrimary -- there was no primary and now there is,
     * just connect the primary.
     * 3) perivous.url !== newPrimary -- primary has changed, d/c from the
     * previous primary and connect to the new one.
     */
    if (!newPrimary || (!previous && newPrimary) ||
        previous.url !== newPrimary) {
        // always close the previous if it exists
        if (previous) {
            log.info({
        ...
        log.info({
            currentPrimary: previous ? previous.url : 'none',
            newPrimary: newPrimary ? newPrimary : 'none'
        }, '_refresh: primary not changed, no updates to the db');
    }
};
