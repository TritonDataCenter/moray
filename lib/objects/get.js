...
function rowToObject(bucket, key, row) {
        ...
        return ({
                ...
                etag: row._etag,
                key: key,
                mtime: new Date(row._mtime),
                value: JSON.parse(row._value)
        });
}


function cacheKey(bucket, key) {
        return ('/' + bucket + '/' + key);
}
...
function checkCache(req, cb) {
        req.cacheKey = cacheKey(req.bucket.name, req.key);
...
function start(req, cb) {
        ...
        req.pgPool.checkout(function (err, client) {
                if (err) {
                        cb(err);
                } else {
                        req.log.debug({
                                pg: client
                        }, 'getObject: transaction started');
                        req.pg = client;
                        cb();
                }
        });
...
function loadObject(req, cb) {
        ...
        req.pg.query(sql, function (err, result) {
                ...
                req.object = rowToObject(bucket, key, result.rows[0]);
