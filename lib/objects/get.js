...
function loadObject(req, cb) {
        ...
        var q;
        var row;
        var sql = sprintf('SELECT _id, _key, _value, _etag, _mtime, _txn_snap' +
        ...
        q.once('end', function () {
                if (!row) {
                        cb(new ObjectNotFoundError(bucket, key));
                } else {
                        req.object = common.rowToObject(bucket, row);
