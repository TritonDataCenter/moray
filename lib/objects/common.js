...
function runPostChain(opts, cb) {
    ...
    var cookie = {
        ...
        update: (opts.previous) ? true : false
    };
    var log = opts.log;
...
// XXX THIS DOES NOT WORK WITH SQLITE.  What they _do_ have is
// BEGIN IMMEDIATE TRANSACTION which prevents any concurrent
// IMMEDIATE transactions.  Right now, we make _every_ transaction
// IMMEDIATE, but in the future we could just do it for put(), etc
function selectForUpdate(opts, cb) {
    ...
    var sql = sprintf('SELECT *, \'%s\' AS req_id FROM %s WHERE ' +
                      '_key=$1 '/* XXX 'FOR UPDATE' */, opts.req_id || 'null', bucket);
