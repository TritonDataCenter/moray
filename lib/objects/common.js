...
function loadBucket(opts, cb) {
    ...
    var log = opts.log;
    var noCache = (opts.opts && opts.opts.noBucketCache);
    ...
    q.once('end', function (result) {
        if (!row) {
            ...
            if (r.reindex_active) {
                opts.bucket.reindex_active = JSON.parse(r.reindex_active);
            }
...
function buildSetLists(dataIndex, bucketIndex) {
    var keys = [];
    var values = [];
    Object.keys(bucketIndex).forEach(function (k) {
        if (dataIndex[k] !== undefined) {
            keys.push(k);
            values.push(dataIndex[k]);
        }
    });
}
