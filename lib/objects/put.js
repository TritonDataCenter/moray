...
function getNextId(req, cb) {
        ...
        var log = req.log;
        var q;
        var sql = sprintf('SELECT id FROM %s_locking_serial FOR UPDATE', b);
...
function update(req, cb) {
        ...
        req.res._etag = _createEtag(req);
        var vals = [req._value, req.res._etag];
        ...
        sql = sprintf('UPDATE %s SET _value=$1, _etag=$2%s, _mtime=%d %s %s ' +
                      'WHERE _key=\'%s\'',
                      req.bucket.name,
                      extraStr,
                      _now(),
...
function updateSerial(req, cb) {
        ...
        sql = sprintf('UPDATE %s_locking_serial SET id=%d', b, id);
        q = req.pg.query(sql);
...
function put(options) {
        ...
        function _put(b, k, v, opts, res) {
                dtrace['putobject-start'].fire(function () {
                        return ([res.msgid, b, k, opts._value]);
                });
