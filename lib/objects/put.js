...
function _mapType(k, v, t) {
    var ret;

    switch (t) {
    case '[boolean]':
    case 'boolean':
        ret = v.toString().toUpperCase();
        break;
...
function _indexObject(schema, object) {
    ...
    Object.keys(schema).forEach(function _index(k) {
        ...
        var t = s.type;
        if (s && v !== undefined) {
            if (/\[number|boolean\]/.test(t)) {
                if (Array.isArray(v)) {
                    var vals = [];
                    v.forEach(function (v2) {
                        vals.push(_mapType(k, v2, t));
                    });
                    ndx[k] = '{' + vals.join(', ') + '}';
                } else {
                    ndx[k] = '{'+ _mapType(k, v, t) + '}';
                }
            } else {
                ndx[k] = _mapType(k, v, t);
            }
...
function getNextId(req, cb) {
    ...
    var sql = sprintf('SELECT id, \'%s\' AS req_id ' +
                      'FROM %s_locking_serial ', /* XXX IMMEDIATE 'FOR UPDATE',*/
                      req.req_id, b);
...
function insert(req, cb) {
    ...
    var values = [req.key, req._value, req.res._etag, Date.now(),
                  req.value.vnode];
    ...
    sql = sprintf('INSERT INTO "%s" (_key, _value, _etag, _mtime, _vnode ' +
                  '%s) VALUES ($1, $2, $3, $4, $5%s)',
    ...
    q.once('end', function () {
        ...
        pg.lastrowid(function (err, id) {
            log.debug(err, 'insert: lastrowid failed');
            if (err) {
                cb(err);
                return;
            }
...
function update(req, cb) {
    ...
    req.res._etag = _createEtag(req);
    var vals = [req._value, req.res._etag, req.value.vnode];
    ...
    Object.keys(req.bucket.index).forEach(function (k) {
        ...
        if (v !== undefined) {
            ...
            case 'boolean':
                vals.push(v.toString().toUpperCase());
                break;
            case '[boolean]':
                if (Array.isArray(v)) {
                    v = '{' + v.map(function (v2) {
            ...
            case '[number]':
                if (Array.isArray(v)) {
                    ...
                    vals.push(v);
                // MORAY-189: PG array that already when through _mapType
                } else if ((v.indexOf('{') === 0) &&
