...
module.exports = {
    ...
    createServer: function createServer(options, callback) {
        ...
        var pgClient = db.createPgClient(config.postgres);
        var dbOpts = {
            pgClient: pgClient,
            log: options.log
        };
        db.createBucketManager(dbOpts, function (err, bucketMgr) {
            if (err)
                return callback(err);

            dbOpts.bucketManager = bucketMgr;
            return db.createObjectManager(dbOpts, function (err2, objectMgr) {
                if (err2)
