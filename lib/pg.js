...
function pgAssert(pg) {
        assert.ok(pg, 'pg handle');
        assert.ok(pg.connection, 'pg connection');
        assert.ok(pg.connection.stream, 'pg stream');
        assert.ok(pg.connection.stream.readable, 'pg readable');
        assert.ok(!pg.connection.stream.destroyed, 'pg not destroyed');
        assert.ok(pg.connection.stream.writable, 'pg writable');
        return (true);
}
...
//                 return (c.client);
//         }).forEach(function (pg) {
//                 console.log(pg._moray_id + ' -> ' + pg.connection.stream.destroyed);
//         });
