...
// Query Timeout error - in short chars, since this is called in a deep
// closure
function QTOErr(sql) {
        this.name = 'QueryTimeoutError';
        this.message = 'query timeout: ' + sql;

        Error.captureStackTrace(this, QTOErr);
}
util.inherits(QTOErr, Error);
...
function PGClient(options) {
                ...
                create: function pgCreate(cb) {
                        ...
                        client.connect(function (err) {
                                ...
                                client.query = function query(c, v, callback) {
                                        ...
                                        var _r = _q.apply(client, arguments);
                                        _r.once('error', _clearTimer);
                                        _r.once('end', _clearTimer);

                                        var _t = setTimeout(function () {
                                                _r.removeListener('end',
                                                                  _clearTimer);
                                                _r.removeListener('error',
                                                                  _clearTimer);
        ...
        this.pool.once('error', this.emit.bind(this, 'error'));
}
