...
PGClient.prototype.checkout = function checkout(callback) {
        ...
        log.debug({pool: self.pool}, 'checkout: entered');
        this.pool.acquire(function (err, client) {
                if (err) {
                        log.debug(err, 'checkout: failed');
                        callback(err);
                } else {
                        log.debug({
                                client: client
...
PGClient.prototype.commit = function commit(client, callback) {
        ...
        var self = this;
        log.debug({client: client}, 'commit: entered');
...
PGClient.prototype.rollback = function rollback(client, callback) {
        ...
        var self = this;
        log.debug({client: client}, 'rollback: entered');
...
PGClient.prototype.start = function start(callback) {
        ...
        this.checkout(function (err, client) {
                if (err) {
                        log.debug(err, 'start: checkout failed');
                        callback(err);
                } else {
                        log.debug({client: client}, 'start: issuing BEGIN');
                        var opts = {
                                client: client,
                                sql: 'BEGIN'
                        }
