...
// We allow exactly one backend at a time (the primary DB instance)
PGResolver.prototype.list = function listBackends() {
    var l = {};
    if (this.key) {
...
function pgAssert(_pg) {
    assert.equal(_pg._moray_txn, false, 'finished transaction');
...
/*
 * node-cueball's checker interface implicitly claims the idle connection
 * we're checking: it expects us to call either handle.release() for
...
function pgCheck(handle, pgc) {
    ...
    req.once('error', function (err) {
        // req will *either* emit 'error', or 'end' but not both.
        pgc.log.trace({ client: pgc, error: err }, 'pgCheck: error');
        handle.close();
    });
    ...
    req.once('end', function () {
        if (!ok) {
            pgc.log.trace({ client: pgc }, 'pgCheck: not OK');
            handle.close();
        } else {
            pgc.log.trace({ client: pgc }, 'pgCheck: OK');
            handle.release();
        }
    });
}
...
function pgCreate(opts) {
    ...
    function _pgCreate(backend) {
        ...
        var onError = function (err) {
            ...
            clearTimeout(pgc._connectTimer);
        ...
        if (opts.connectTimeout > 0) {
            pgc._connectTimer = setTimeout(function () {
        ...
        client.once('connect', function onConnect() {
            ...
            /*
             * We only want to remove the listener we added (onError).  The
             * PGClient constructor also has an error listener on client, which
             * we are using to forward from client to PGClient.
             */
...
function PGPool(options) {
     ...
     */
    this.pool = new cueball.ConnectionPool({
        domain: 'dummy.host', // dummy value not used.
        service: '_postgres._tcp', // dummy value not used.
...
PGPool.prototype.checkout = function checkout(callback) {
    ...
    this.pool.claim(function claimFromPool(err, handle, client) {
        if (err) {
            log.trace(err, 'checkout: failed');
            callback(err);
        } else {
            /*
             * Save the pool-handle into our client object: (callbacks
