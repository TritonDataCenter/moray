...
function consolidateReindex(old, ver, fields) {
    ...
    fields.forEach(function (field) {
        if (result[ver].indexOf(field) === -1) {
            result[ver].push(field);
        }
    });
    return result;
}
...
function loadBucket(req, cb) {
    ...
    q.once('end', function (r) {
        ...
        // Requesting a column reindex on a versionless bucket is not allowed
        if (req.opts.reindex && v2 === 0) {
            cb(new InvalidBucketConfigError(
                        'cannot reindex versionless bucket'));
            return;
        }
...
function ensureReindexProperty(req, cb) {
    if (req.opts.reindex) {
        if (!req.raw_bucket.hasOwnProperty('reindex_active')) {
            ...
            req.log.warn('raw bucket', req.raw_bucket);
        }
    }
    cb(null);
}
...
function ensureRowVer(req, cb) {
    ...
    if (!req.opts.reindex) {
        // unnecessary if reindex not requested
        cb(null);
        return;
    }
...
function clearCache(req, cb) {
    // The loadBucket function used for object-related actions has an internal
    // bucket cache.  The entry for this bucket should be invalidated.
    objCommon.clearBucketCache(req.bucket.name);
    cb(null);
}
