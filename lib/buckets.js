...
function create(req, res, next) {
    ...
    function invalidIndexFormat(index) {
        return next(new InvalidArgumentError('index \'%s\' is an invalid format',
                                             util.inspect(index)));
    }
        ...
        if (typeof (req.params.index) === 'string') {
            ...
            for (i = 0; i < req.params.index.length; i++) {
                if (typeof (req.params.index[i]) !== 'string')
                    return invalidIndexFormat(req.params.index[i]);
            ...
            for (i = 0; i < indexKeys.length; i++) {
                var ndx = req.params.index[indexKeys[i]];
                if (typeof (ndx) !== 'object' ||
                    typeof (ndx.type) !== 'string' ||
                    !(ndx.type in INDEX_TYPES)) {

                    return invalidIndexFormat(ndx);
                }
            }
            indexes = req.params.index;
        } else {
            return invalidIndexFormat(req.params.index);
        }
    }
