    ...
    this.pool = poolModule.Pool({
        ...
        create: function create(callback) {
            ...
            client.connect(function (err) {
                ...
                client.commit = function pgCommit(cb) {
                ...
                client.rollback = function pgRollback(cb) {
                    assertFunction('callback', cb);
...
Postgres.prototype.checkout = function checkout(callback) {
    ...
    var self = this;
    this.log.debug('checkout called');
...
Postgres.prototype.release = function release(client) {
    ...
    var released = client._inuse;
    this.log.debug({inuse: client._inuse || false}, 'release called');
 ...
 */
Postgres.prototype.start = function start(callback) {
    assertFunction('callback', callback);

    var self = this;
    return this.checkout(function (err, client) {
        if (err)
            return callback(internalError(err));

        return self._query(client, 'BEGIN', function (err2) {
            if (err2)
