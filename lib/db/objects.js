...
function compileQuery(bucket, schema, query, child) {
    ...
    case 'not':
        if ((val = compileQuery(bucket, schema, f.filter.toString(), true)))
            where += ' (NOT (' + val + '))';

        break;

    case 'ge':
        if ((val = value(f.attribute, f.value)))
 ...
 * @arg {Object} opts       - (optional) etag to enforce
 ...
 */
ObjectManager.prototype.put = function put(bucket, key, value, opts, callback) {
    assertString('bucket', bucket);
    assertString('key', key);
    ...
    var log = this.log;
    var pg;
    var _release = opts.pgClient ? false : true;
    var save;
    var self = this;
    var tombstone = true;
    ...
    return async.series([
        ...
        function enforceEtag(cb) {
            if (opts.value === undefined || !save)
                return cb();

            if (save.length === 0)
                return cb(new EtagConflictError(bucket, key, 'does not exist'));

            var matches = save[0].etag === opts.value;
            if (matches !== opts.match)
        ...
        function insert(cb) {
            ...
            return pg.query(sql, values, function (err, rows) {
                if (err)
                    return cb(err);
        ...
        function commit(cb) {
            return pg.commit(_release, cb);
        }
    ], function (err) {
        if (err) {
            if (pg) {
                return pg.rollback(_release, function () {
                    return callback(err);
                });
            }
            return callback(err);
 ...
 * @arg {Function} callback - function (err).
 */
ObjectManager.prototype.del = function del(bucket, key, opts, callback) {
    assertString('bucket', bucket);
    assertString('key', key);
    if (typeof (opts) === 'function') {
        callback = opts;
...
ObjectManager.prototype.find = function find(bucket, query, opts, callback) {
    assertString('bucket', bucket);
    assertString('query', query);
    if (typeof (opts) === 'function') {
        callback = opts;
    ...
    return async.series([
        ...
        function buildWhereClause(cb) {
            try {
                ...
                if (opts.sort) {
                    var res = /^(\+|\-)?(\w+)/.exec(opts.sort);
                    if (res.length >= 3) {
                        clause += ' ORDER BY ' + res[2];
                        clause += res[1] === '-' ? ' DESC' : ' ASC';
                    }
                }
            } catch (e) {
                return cb(e);
