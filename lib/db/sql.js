...
function UPDATE_CONFIG() { /*
    UPDATE buckets_config SET schema='%s', pre='%s', post='%s' WHERE name='%s'
                           */
}
...
function GET_ENTRY() { /*
    SELECT key, value, etag, mtime FROM %s_entry WHERE key='%s'
                        */
}
...
function FIND_ENTRIES() { /*
    SELECT key, value, etag, mtime FROM %s_entry WHERE %s
                           */
}
