...
function translateBuckets(rows) {
    assertObject('rows', rows);

    var obj = {};
    rows.forEach(function (r) {
        obj[r.name] = JSON.parse(r.schema);
    });
    ...
    return async.series([
        ...
        function checkIfExists(cb) {
            ...
            return pg.query(sql, function (err, rows) {
                ...
                if (rows.length > 0)
                    old = rows[0];
                // return cb(new BucketAlreadyExistsError(bucket));
        ...
        function updateBucketConfig(cb) {
            ...
            try {
                var post = JSON.stringify(options.post);
                var pre = JSON.stringify(options.pre);
...
BucketManager.prototype.get = function get(bucket, callback) {
    ...
    if (this._buckets[bucket])
        return callback(null, this._buckets[bucket]);
