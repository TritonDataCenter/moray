    ...
    return this.pgClient.query(SQL.LIST_BUCKETS, function (err, rows) {
        ...
        try {
            ...
            rows.forEach(function (r) {
                ...
                for (i = 0; i < buckets[r.name].pre.length; i++) {
                    buckets[r.name].pre[i] =
                        eval('fn=' + buckets[r.name].pre[i]);
                }

                for (i = 0; i < buckets[r.name].post.length; i++) {
                    buckets[r.name].post[i] =
                        eval('fn=' + buckets[r.name].post[i]);
                }
            });
            if (fn) {
                fn = null;
...
BucketManager.prototype.get = function get(bucket, callback) {
    ...
    if ((b = this.bucketCache.get(bucket))) {
        log.debug('BucketManager::get %s => found in cache', b);
